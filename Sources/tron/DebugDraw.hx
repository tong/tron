...
class DebugDraw {
	...
	var midPoint = new Vec4();
	var midLine = new Vec4();
	var corner1 = new Vec4();
	var corner2 = new Vec4();
	var corner3 = new Vec4();
	var corner4 = new Vec4();
	var cameraLook = new Vec4();

	public var color : kha.Color = 0xff0000ff;
	public var strength = 0.011;
	...
	public function new() {
		...
		#if arm_deferred
		pipeline.fragmentShader = kha.Shaders.debug_line_deferred_frag;
		#else
		pipeline.fragmentShader = kha.Shaders.debug_line_frag;
		#end

		pipeline.vertexShader = kha.Shaders.debug_line_vert;
	...
	public function notifyOnRender( f : Void->Void ) {
		//if( inst == null ) new DebugDraw();
		iron.RenderPath.notifyOnContext("mesh", (g4: kha.graphics4.Graphics, i: Int, len: Int) -> {
			g = g4;
			if( i == 0 ) begin();
			f();
			if( i == len - 1 ) end();
	...
	public function bounds( transform: iron.object.Transform ) {
		...
		final dx = transform.dim.x / 2;
		final dy = transform.dim.y / 2;
		final dz = transform.dim.z / 2;
